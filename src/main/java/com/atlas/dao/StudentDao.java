...
public class StudentDao {
    ...
    public void save(Student s) {
        ...
        item.put("enrolledCourseIds", AttributeValue.builder().l(s.getEnrolledCourseIds().stream().map(id -> AttributeValue.builder().s(id).build()).collect(Collectors.toList())).build());
        item.put("waitlistedCourseIds", AttributeValue.builder().l(s.getWaitlistedCourseIds().stream().map(id -> AttributeValue.builder().s(id).build()).collect(Collectors.toList())).build());
        client.putItem(PutItemRequest.builder().tableName(table).item(item).build());
    }

    public Student getById(String id) {
        GetItemResponse resp = client.getItem(GetItemRequest.builder().tableName(table).key(Map.of("id", AttributeValue.builder().s(id).build())).build());
    ...
    public Student findByEmail(String email) {
        // simple scan to enforce unique email (ok for demo)
        ScanResponse resp = client.scan(ScanRequest.builder().tableName(table).filterExpression("email = :e").expressionAttributeValues(Map.of(":e", AttributeValue.builder().s(email).build())).build());
